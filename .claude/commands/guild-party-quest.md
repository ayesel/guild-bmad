...
Invoke a Claude subagent to run `/guild-agent-sage (DSF / design-system-foundation)`. Sage audits:
...
# Launch Claude, brief it to run /guild-agent-ranger (VA / visual-audit) on competitor products
...
Invoke a subagent to run `/guild-agent-ranger (RS / research-synthesis)`. Feed it:
...
**Claude:** Run `/guild-agent-sage (PR / pre-handoff)` quality gate.
...
Invoke subagent → `/guild-agent-healer (UX / ux-spec)`.

### Step 13: Generate Stories — Claude
Invoke subagent → `/guild-agent-healer (JS / jira-stories)`.
