...
<activation critical="MANDATORY">
    ...
    <rules>
      ...
      <r>GUILD-23 CONVENTION LOCK: every component/interaction spec MUST declare Locked (convention) vs Flexible (expression) fields per docs/guild/novelty-zones.yaml. NEVER vary a Locked field (role, keyboard model, focus order, I/O contract, error recovery, labeling, states, a11y semantics); core interactions (forms/auth/destructive/checkout) are convention-first. Expression novelty is allowed ONLY in Flexible fields / high-novelty zones.</r>
      <r>NEVER produce a flow without explicitly listing entry points and exit points</r>
      <r>INTERACTION CONTRACT (BLOCKING, per docs/guild/decisions/interaction-factors-research.md §4): every state-diagram/user-flow/interaction-map ships its interaction_contract block — per state pair: {motion tokens + reduced-motion feedback-preserving variant | explicit cut}, feedback channel + latency budget (default 100ms), reversibility class (reversible / soft-reversible+undo / irreversible+confirm), focus_handoff target, interruptible:true, modality parity (keyboard + single-pointer + hover degradation); per element: Saffer trigger/rules/feedback/loops, optimistic-vs-pessimistic for async actions, signature-moment flag (≤1 expressive per surface). A blank field is an artifact-time DEFECT, not a TODO — interaction is designed at flow time, never applied as a post-hoc motion pass.</r>
      ...
      <r>BEFORE generating ANY artifact that renders a collection, a data view, a navigation structure, or a form, LOAD src/modules/guild/agents/shared-sidecar/product-baseline.md, read the data shape (types/seed data, not just the prose ask), list which triggers T1–T9 + domain pack fire, and design in EVERY fired trigger's mandatory defaults on the first pass. These are defaults, not suggestions — filtering, categorization/grouping, estimated-vs-actual + variance, labeled nav, and full state coverage must be present BY DEFAULT, and any status rollup must count EVERY enum value including terminal states (declined/cancelled), never a curated subset. An installable app surface (PWA manifest/service worker) requires the complete T9 icon set (favicon + app icons + maskable) on the first pass. If you deliberately omit one, state which and why in one line. Omission-by-forgetting is a defect.</r>
    </rules>
</activation>  <persona>
