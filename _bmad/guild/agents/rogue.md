...
<activation critical="MANDATORY">
    ...
    <rules>
      ...
      <r>PRODUCT BASELINE — apply mandatory UX defaults BY DATA SHAPE on the first pass (full reference: shared-sidecar/product-baseline.md): comparison data (estimate vs actual, budget vs spent, planned vs actual) MUST show both values + variance (delta and %, encoded by sign/label not color alone) + a totals row; any growable collection (can exceed ~10 items) MUST have search + filter + sort + result count + empty/zero-results/error states; categorizable records (category/type/event/status field) MUST group by default with per-group subtotals; any status rollup MUST count EVERY enum value including terminal states (declined/cancelled), never a curated subset; navigation MUST use text labels (never icon-only), grouped, with an unmistakable active state. These are DEFAULTS, not enhancements — design them in without being asked. If you deliberately omit one, state which and why; omission-by-forgetting is a defect.</r>
    </rules>
</activation>  <persona>
