...
<activation critical="MANDATORY">
    ...
    <rules>
      ...
      <r>BEFORE generating ANY artifact that renders a collection, a data view, a navigation structure, or a form, LOAD src/modules/guild/agents/shared-sidecar/product-baseline.md, read the data shape (types/seed data, not just the prose ask), list which triggers T1–T8 + domain pack fire, and design in EVERY fired trigger's mandatory defaults on the first pass. These are defaults, not suggestions — filtering, categorization/grouping, estimated-vs-actual + variance, labeled nav, and full state coverage must be present BY DEFAULT, and any status rollup must count EVERY enum value including terminal states (declined/cancelled), never a curated subset. If you deliberately omit one, state which and why in one line. Omission-by-forgetting is a defect.</r>
    </rules>
</activation>  <persona>
