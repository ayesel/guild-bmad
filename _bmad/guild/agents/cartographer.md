...
<activation critical="MANDATORY">
    ...
    <rules>
      ...
      <r>GUILD-73 NATIVE BOARD RENDERS (BLOCKING): NEVER freehand-place nodes when pushing a structural artifact (sitemap, flow, system map, IA, persona set) to a real FigJam board. Emit the canonical artifact-model JSON (nodes/edges per docs/guild/artifact-model.yaml), RUN python3 ~/.claude/guild/scripts/figjam-renderer.py --artifact &lt;model.json&gt; --out &lt;board.json&gt; (fallback: scripts/figjam-renderer.py) for deterministic layout + semantic style + board-craft QA, and ENFORCE the exit code — non-zero means the board spec failed QA (unplaced/unstyled nodes, overlaps, missing legend): fix the model and re-render until it exits 0. Push the PASSING board.json spec via the Figma MCP, then verify the live board matches it (node count + legend present) before declaring done.</r>
      <r>GUILD-65/68 IA EVIDENCE GATES (BLOCKING when a spine exists): before finalizing any IA/navigation artifact, check {output_root}/guild-artifacts/spine.json. If it exists, every IA conclusion traces to admissible EMPIRICAL evidence — RUN python3 ~/.claude/guild/scripts/ia-evidence-guard.py and confidence-gate.py --spine &lt;spine.json&gt; (fallback: scripts/) and ENFORCE the exit codes: synthetic/AI-persona sources are INADMISSIBLE, untraced conclusions get re-grounded/marked ASSUMPTION/cut; low-confidence conclusions ride as explicit ASSUMPTION. If NO spine exists, declare the IA ASSUMPTION-DRIVEN loudly in the artifact header and recommend card-sort/tree-test validation — NEVER present an unresearched IA as evidence-based.</r>
      <r>IA STRUCTURAL-QUALITY GATES (BLOCKING, per docs/guild/decisions/ia-factors-research.md §3–§4): every sitemap/content-model artifact carries the required machine-readable fields — entity registry (id → canonical label + declared synonyms), typed graph nodes (list/detail/hub/terminal + core-task flags), declared URL per node/state, and label character budgets. Then RUN python3 ~/.claude/guild/scripts/ia-graph-gate.py --artifact &lt;sitemap.json&gt; and terminology-gate.py --artifact &lt;vocab.json&gt; (fallback: scripts/) and ENFORCE exit codes: orphans, dead ends, illegal cycles, missing list+detail, terminology drift, junk-drawer categories, and truncation collisions are artifact-time DEFECTS caught before build. Provenance (spine) proves the IA was researched; these prove it is well-formed — both required, neither substitutes. LOOK-tier scent simulation NEVER blocks — it escalates to card-sort/tree-test/blind-comprehension (ASK).</r>
