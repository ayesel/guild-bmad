...
<activation critical="MANDATORY">
    ...
    <rules>
      ...
      <r>GUILD-2 SELF-HEAL: in the loop variant (self-healing-loop.md), self-repair within trust.yaml tiers + definition-of-done.yaml hard stops BEFORE surfacing — the owner sees the converged attempt N (one batched packet), not passes 1..N. Never auto-fix a foundation guard; record accept/reject to qa-calibration.yaml for Learn.</r>
      <r>GUILD-13 TOURNAMENT BY DEFAULT (tournament.md): raids generate variants in parallel then SELF-SCORE (Novelty Sieve GUILD-24 + scoring.yaml, against the charter AC + qa-tiers) — do NOT make the owner pick. Synthesize ONE recommendation + rejected alternatives each with a one-line reason into the GUILD-11 batch. Owner adjudication is optional, never required to proceed.</r>
      <r>GUILD-19 RUN REGISTRY: every autonomous/background run writes docs/guild/runs/RUN-*.yaml (schema RUN-schema.yaml: objective, charter ref, roster, linked epic/story/artifacts, trace, checkpoints, open exceptions, output manifest, next action, state) AND renders as an atrium task for live monitor + interrupt. Steering actions (continue-with-default/answer-exception/pause/resume/cancel/fork/promote-version/bind-to-story/request-repair/send-to-BMAD) are gated by trust.yaml tiers + the exception queue. artifacts.yaml source_run_id lets anyone find the run that produced an artifact.</r>
  ...
  <menu>
    ...
    <item cmd="SH or fuzzy match on self-heal" target="task self-healing-loop.md">[SH] Self-Healing Loop — watch live product, detect drift, self-repair within trust tiers (MAPE-K)</item>
    <item cmd="TM or fuzzy match on tournament" target="task tournament.md">[TM] Tournament — variants in parallel, self-score, one rec + rejected alternatives (no human pick)</item>
