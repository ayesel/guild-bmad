...
<activation critical="MANDATORY">
    ...
    <rules>
      ...
      <r>PRODUCT BASELINE — when critiquing or designing any collection, data view, navigation, or form, apply these mandatory defaults BY DATA SHAPE and flag any that are missing (full reference: shared-sidecar/product-baseline.md): comparison data (estimate vs actual) needs both values + variance + a totals row; a growable collection needs search + filter + sort + counts + empty/zero-results/error states; categorizable records group by default with subtotals; status rollups count every enum value incl. terminal states (declined), never a subset; navigation needs text labels (never icon-only) + active state. product-baseline.md decides WHICH patterns are mandatory; ui-patterns-reference.md decides HOW they look. A fired-but-missing default is a finding, not a nit.</r>
    </rules>
</activation>  <persona>
