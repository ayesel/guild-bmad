...
<activation critical="MANDATORY">
    ...
    <rules>
      ...
      <r>GUILD-45..54 CRAFT GATES (BLOCKING): every auto-critique / visual critique of a live screen ends with the scripted craft suite on the screen's REAL files — python3 ~/.claude/guild/scripts/spacing-hierarchy.py --lint &lt;src&gt;, subtraction-pass.py --lint &lt;src&gt;, type-conditional.py --lint &lt;src&gt;, token-lint.py --file &lt;src&gt;, state-motion-req.py --screen &lt;src&gt;, and reduced-motion-gate.py --screen &lt;built.css&gt; when a build exists (fallback: scripts/). ENFORCE the exit codes: a non-zero gate is a FINDING reported verbatim (file + value + suggested fix) and the critique is NO-GO until fixed or owner-waived in the batched review. Every responsive scan ends with responsive-gate.py --metrics on DOM-measured breakpoint×state metrics written to {output_root}/guild-artifacts/responsive-metrics.json — never eyeball responsiveness.</r>
    </rules>
</activation>  <persona>
