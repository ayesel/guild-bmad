...
- **No decorative side rails (owner ban, 2026-07-02)** — colored left-border accent rails on cards/list rows (the Claude-Design house habit) are PROHIBITED as a default. "There's a time and a place": a rail is a lazy emphasis channel that adds a competing device to every card, breaks silently at the token layer, and inverts severity when a fallback fires. Encode state with labeled chips/badges, grouping, or type weight instead. A side rail is permissible ONLY when the owner explicitly asks for one — and then on one card class, never systemwide. (Learned 2026-07-02, GUILD Hall: an undefined `--denim` rail token silently gave FYI cards the brightest rail on the page.)
...
- **Banned AI-slop fingerprints (standing negative constraints, anti-slop-survey.md 2026-07-06)** — these are the aesthetic tells of ungoverned generation and are banned BY DEFAULT at generation time (controlled evidence: negative constraint lists are the highest-impact, safest context artifact — stronger than any positive style guidance): Inter/Roboto/Arial as the hero/display face · vibe-purple (#8B5CF6-family) gradients and purple→blue hero washes · glassmorphism as a default surface treatment · colored left-border cards (already owner-banned above) · icon-topped card grids · a badge/pill floating above the hero H1 · 01/02/03 numbered rows for non-sequential content · bounce/elastic easing on standard transitions · card-in-card nesting · one identical drop shadow on everything · emoji as functional icons. Un-banning any of these is an explicit owner call recorded in the design-direction brief, never a silent default. Verified post-render by `slop-fingerprint-gate.py` — deterministic checks only, never LLM-judged.
