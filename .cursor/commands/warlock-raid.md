---
name: warlock-raid
description: "3-model raid for Warlock (Content Strategist). Runs the same UX writing task across Claude, Codex, and Gemini, then runs a diverse-jury tournament to SELECT the strongest copy — recommendation + rejected alternatives, NO merge (GUILD-85: merging copy averages away distinctive voice; proven by the GUILD-42 A/B, SELECT > MERGE). Use for microcopy, error messages, voice/tone, naming. Requires atrium (ATRIUM=1 env var)."
user-invocable: true
...
**Do not blend the 3 copy sets into a "best-of."** Merging copy averages away the
distinctive voice (proven by the GUILD-42 A/B: SELECT > MERGE). Instead run the
tournament:

1. Treat the 3 model outputs as **candidates** (diverge — keep each voice whole).
...
3. Return the **WINNER VERBATIM** as the recommendation — do NOT rewrite or blend it —
   plus the others as **rejected alternatives**, each with a one-line reason + its
   distinctive strength noted (so a bold voice isn't lost).

> Multi-model raids stay valuable for **QA / research COVERAGE** (Ranger/Sage). Only the
> CREATIVE merge (Mage/Warlock) is replaced by SELECT here.

```markdown
---
artifact: warlock-raid-selection
author: Warlock (3-Model Raid — SELECT, no merge)
...
## Candidates
| Model | Voice / approach (1 line) | Quality | Novelty | Unique strength |
|---|---|---|---|---|
| Claude | … | … | … | … |
| Codex | … | … | … | … |
| Gemini | … | … | … | … |
...
[the winning candidate's copy set, unedited — microcopy, error messages, labels as that model wrote them]
...
Save to: `{output_root}/guild-artifacts/warlock-raid-[topic].md`
