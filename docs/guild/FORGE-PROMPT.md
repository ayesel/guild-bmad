...
**Friction wave SHIPPED (698f09c, 2026-07-01 — every fix from the live Nourish speedrun):** state-motion-req exits non-zero + Tailwind-aware + --globals (false-positive wall killed); responsive-capture.mjs + responsive-metrics.browser.js = one-command breakpoint metrics (proven live); perf-budget-gate .next-aware discovery; /guild-spine-backfill command; product-baseline **T9 installable-icon-set trigger** wired T1–T9 into Rogue/Mage/Sage.
**Known real findings awaiting fixes:** wedding-hub fidelity NO-GO (register its DTCG export) + responsive NO-GO (23px nav targets, 88×16 link, 167ch measure); Nourish D3 (hover/transition on 4 static components) + D4 (register DTCG) pending owner; perf-budget needs per-project budget overrides + framework budget profiles (30kb JS budget is for generated artifacts, not app bundles) and external-dep detection false-positives on URL strings inside bundled JS; Nourish suite is load-sensitive (documented flaky-shared-suite class — timeout-storm under parallel machine load; green quiet/isolated) → Stage-2 TEA burn-in + heavy-test thinning per the app's own CLAUDE.md discipline.
...
3. ~~**Comment→regenerate reflex**~~ — SHIPPED 2026-07-01 (c074156, card f896cbc2): /guild-comment + regenerate-note.py (rendered-pixels pick notes) + regenerate-pick.py (apply + taste capture, dry-run proven). REAL PROOF on Nourish D3: 3 motion treatments, all gates 0, suites 859/859 ×3 — owner's pick waiting on the delivered note.
4. ~~**Widget, real → GUILD HALL**~~ — HALL v1 SHIPPED 2026-07-02 (6da94a1, card faad5703): delegated-work inbox app on the deep-research mental model (ui-mental-model-research.md), blind-comprehension GATED (round-1 FAIL → redesign → round-2 PASS, hall-blind-test.md), REAL write channel (verdicts + picks w/ taste capture, dry-run proven live). Atrium note = thin pointer. REMAINING: owner adoption call, run-exception plain-language pass, pane-API embed (026d669e final phase).
...
6. ~~**Proactive brain**~~ — SHIPPED 2026-07-02 (c3eae68, cards cfff66c9/f18c0a39/605313e1 In Progress w/ evidence): pattern-store (14 real patterns, operator-owned), affordances.yaml + affordance-check (proven: Vendors EXIT 0, house-cup EXIT 1 w/ 9 gaps), equivalence-check (house-cup: 35 cancels/20 implementations), Rogue/Mage BLANK-PAGE CONTRACT, and guild-render spec-once fan-out (FigJam native + HTML prototype from one model — proven on the insights brief). Remaining on cards: live usage hardening + more harvests.
7. ~~**Wedding-hub re-test**~~ — RUN 2026-07-06 (RUN-2026-07-06-accountability.yaml in the wedding-hub workspace, commit 5ecda8f): THESIS HOLDS on the deterministic layer — M1 affordance class caught cold (11 findings across 5 screens, zero prompting; the fixed screen passes, so the gate discriminates), M2 equivalence caught, 3 NEW expert-grade classes surfaced unprompted (terminology drift, route-focus-to-body, unshareable detail state). Residue: TIME driving harness unbuilt; responsive/fidelity NO-GOs remain open FIXES, not detection gaps; 2 new-gate calibration items filed. Owner triage of the waive-or-fix list = calibration labels (starts the counter).
...
- **Guild's surfaces obey Guild's baseline (owner, 2026-07-02: "I shouldn't have to tell you this"):** any owner-facing surface shipped for Guild itself (Hall, widget, pick notes) must apply the product-baseline triggers AT BUILD TIME — a collection gets filters + view toggle + pagination/grouping the moment it can exceed ~10 items (T2), every item gets visible identity (icon + category, never N identical cards), navigation always has a way back. Before calling a surface done, run its own gates against it (guild-suggest / affordance-check on the rendered page) — the Hall shipping a 104-item list with no filters was a defect the owner had to catch.
