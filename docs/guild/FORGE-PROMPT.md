...
4. ~~**Widget, real**~~ — SHIPPED 2026-07-01 as the canvas-note widget (card 026d669e): `scripts/widget-feed.py` (data layer) + `scripts/guild-widget.py` (renderer, converged Now/Journey/Library design), live against nourish + the engine repo, "Guild Dashboard" workspace-command repointed. REMAINING on the card: pane-API embed when Jonny's API lands, per `docs/guild/decisions/widget-more-packet.md` (exploration recommends GUILD HALL — standalone cockpit, atrium as embed adapter — OWNER DECISION PENDING).
